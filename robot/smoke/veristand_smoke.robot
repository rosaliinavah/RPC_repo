...
Verify Veristand API using Engine Demo Project
    ...
    ${act_rpm} =    VeristandApi.Read Channel Value    channel_name=ActualRPM
    ${des_rpm} =    VeristandApi.Read Channel Value    channel_name=DesiredRPM
    Builtin.Should Be Equal As Numbers    first=${act_rpm}    second=${0.0}
    Builtin.Should Be Equal As Numbers    first=${des_rpm}    second=${0.0}

    ${rpm_acc} =    VeristandApi.Read Channel Value    channel_name=Engine Calculations/RPM Acceleration
    Builtin.Should Be Equal As Numbers    first=${rpm_acc}    second=${0.0}

    VeristandApi.Write Channel Value    channel_name=DesiredRPM    value=0
    VeristandApi.Write Channel Value    channel_name=EnginePower    value=1
    BuiltIn.Sleep    2s    reason=Need to wait to get value written in system

    @{values} =    VeristandApi.Read Channels Values
    ...    ActualRPM
    ...    DesiredRPM
    ...
    VeristandApi.Write Channels Values    DesiredRPM    1000.5
    BuiltIn.Sleep    1s    reason=Need to wait to get value written in system
    @{values} =    VeristandApi.Read Channels Values    DesiredRPM
    Builtin.Should Be Equal As Numbers    first=${values[0]}    second=${1000.5}

    VeristandApi.Write Channel Fault    channel_name=ActualRPM    value=0.5
    BuiltIn.Sleep    1s    reason=Need to wait to get value written in system
    ${act_rpm} =    VeristandApi.Read Channel Value    channel_name=ActualRPM
    Builtin.Should Be Equal As Numbers    first=${act_rpm}    second=${0.5}

    VeristandApi.Clear Channel Fault    channel_name=ActualRPM
    BuiltIn.Sleep    1s    reason=Need to wait to get value written in system
    ${act_rpm} =    VeristandApi.Read Channel Value    ActualRPM
...
Teardown Bench Local
    ...
    VeristandApi.Disconnect System And Undeploy Definition    undeploy=${True}
