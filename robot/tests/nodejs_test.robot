...
Test NodeJs Webserver
    ...
    #Start Process    node    server_with_db.js    shell=True
    Start Process
    ...    node
    ...    server_with_db.js
    ...    cwd=C:\\Codesys RPC
    ...
    WHILE    $update_values == 'TRUE'
        ...
        ${INT_VARIABLE} =     CodesysRpcClient.Read Variable    ${PLC_INT_VARIABLE}
        ${DWORD_VARIABLE} =   CodesysRpcClient.Read Variable    ${PLC_DWORD_VARIABLE}
        ${BYTE_VARIABLE} =    CodesysRpcClient.Read Variable    ${PLC_BYTE_VARIABLE}
        #${BOOL_VARIABLE} =    CodesysRpcClient.Read Variable    ${PLC_BOOL_VARIABLE}

        ${json} =    Set Variable
        ...    {"integer_value": ${INT_VARIABLE}, "dword_value": ${DWORD_VARIABLE}, "byte_value": ${BYTE_VARIABLE}}     #, "boolean_value": ${BOOL_VARIABLE}}

        Create File    data.json    ${json}
